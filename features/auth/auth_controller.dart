...
class AuthController extends GetxController {
  ...
  Future<void> signUp() async {
    ...
    final confirmPassError = Validator.validateConfirmPassword(
      password,
      confirmPassword,
    );
    ...
    if (confirmPassError != null) {
      return ErrorHandler.showErrorSnackBar(confirmPassError);
    }
  ...
  Future<void> startProfilePhoneVerification(String phoneNumber) async {
    final normalizedPhone = _sanitizePhoneNumber(phoneNumber.trim());
    final phoneError = Validator.validatePhoneNumber(normalizedPhone);
    if (phoneError != null) return ErrorHandler.showErrorSnackBar(phoneError);

    try {
      isOtpSending.value = true;
      final args = await _sendOtp(
        flowType: OtpFlowType.profilePhoneVerification,
      ...
      await Get.toNamed(Routes.OTP_VERIFICATION, arguments: args);
    } on FirebaseAuthException catch (e) {
      ErrorHandler.showErrorSnackBar(e);
    } catch (e) {
      ErrorHandler.showErrorSnackBar(e.toString());
  ...
  Future<void> verifyOtp(OtpFlowArgs args) async {
    ...
    try {
      ...
      if (args.isSignUp) {
        ...
        ErrorHandler.showSuccessSnackBar(
          "Account Created",
          "Your parent account is verified",
        );
        Get.offAllNamed(Routes.DASHBOARD);
        return;
      }

      if (args.isProfilePhoneVerification) {
        ...
        await _authRepository.attachPhoneCredential(credential);

        final existingUser = await _userRepository.getUser(currentUser.uid);
        ...
        _clearAuthInputs(clearPassword: false, clearOtp: true);
        ErrorHandler.showSuccessSnackBar(
          "Phone Verified",
          "Your phone number can now be used for login",
        );
      ...
      if (signedInUser == null) {
        throw Exception('Phone login failed. Please try again.');
      }

      final appUser = await _userRepository.getUser(signedInUser.uid);
      if (appUser == null ||
          !appUser.isPhoneVerified ||
          appUser.phone != args.phoneNumber) {
        await _authRepository.signOut();
        throw FirebaseAuthException(
          code: 'phone-not-verified',
          message: 'This phone number is not verified for any profile.',
        );
      }
  ...
  Future<OtpFlowArgs> _sendOtp({
    ...
    await _authRepository.verifyPhoneNumber(
      ...
      verificationCompleted: (credential) {
        dev.log(
          "Phone verification auto-completed for $phoneNumber",
      ...
      codeAutoRetrievalTimeout: (verificationId) {
        dev.log(
          "OTP auto retrieval timed out for $phoneNumber",
          name: "AUTH_CONTROLLER",
        );
      },
    );
  ...
  void _clearAuthInputs({bool clearPassword = true, bool clearOtp = false}) {
  ...
  bool get isEmailVerified =>
      _authRepository.currentUser?.emailVerified ?? false;
}
